...
export default function LimpezaInfoPedido({ route }) {
    //<script>
    ...
    async function StatusFunction(e) {
        ...
        await updateDoc(doc(db, "Limpeza", IdUpdate), {
            status: e.item
        }).then(navigation.navigate("Limpeza"))
    ...
    function OpenPlant() {
        if (plant == false) {
            setPlant(true)
            navigation.navigate("LimpezaHomeEmpreendimento", {
                Emp: Empreendimento,
            })

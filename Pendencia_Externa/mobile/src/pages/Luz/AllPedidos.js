...
export default function AllPedidos(props) {
    //<script>
    //NAVIGATION
    ...
    pedido.forEach((item) => {
        ...
        if (Select_Empreendimento == "Todos") {
            Selected.push(item)
        }

        if (item.empreendimento == Select_Empreendimento) {
            Selected.push(item)
        }
    })
        ...
        <View className='text-white bg-[#333333] w-full h-full flex-col flex-1 my-0 mx-auto max-w-[1200px]'>
            ...
            <ScrollView className="px-[2px]">
                ...
                {Selected.map((item, index) => (
                    <LuzPedido_List IdMaster={item.id} imagem={item.imagem} cor={item.cor} navigation={navigation} key={index} id={index} empreendimento={item.empreendimento} quadra={item.quadra} numero={item.numero} status={item.status} pedido={item.pedido} />

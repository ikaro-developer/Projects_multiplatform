...
//COMPONENT
import LuzPedido_List from './LuzPedido_List'


export default function NotConcluido(props) {
     //<script>
  ...
  const Disp_empreendimento = ["Todos", "Jurema","Boa Esperança"]
  ...
  pedido.forEach((item) => {
    if (Select_Empreendimento == "Todos" && item.status=="Solicitado") {
      Selected.push(item)
    }

    if (item.empreendimento == Select_Empreendimento  && item.status=="Solicitado") {
      Selected.push(item)
    }
  })
...
{Selected.map((item, index) => (
  <LuzPedido_List imagem={item.imagem}  IdMaster={item.id} navigation={navigation} key={index} id={index} empreendimento={item.empreendimento} quadra={item.quadra} numero={item.numero} status={item.status} pedido={item.pedido} />

...
export default function EmAndamento(props) {
     //<script>
  ...
  pedido.forEach((item) => {
    if (Select_Empreendimento == "Todos" && item.status=="Iniciando") {
      Selected.push(item)
    }

    if (item.empreendimento == Select_Empreendimento  && item.status=="Iniciando") {
      Selected.push(item)
    }
  })
